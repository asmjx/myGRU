...
class Process():
    def __init__(self):
        ...
        self.network = testLSTM(self.feature_size+self.position_encoding_size)
            ...
            for temp_index in range(0,-1,-1):
                ...
                for i in range(10,temp_idx.shape[0],self.batch_size):
                    ...
                    for j in range(min(self.batch_size,temp_idx.shape[0]-i)):
                        ...
                        batch_data_list.append(Variable(torch.from_numpy(one_feed_data).type(torch.FloatTensor)))
                        batch_position.append(Variable(torch.from_numpy(position).type(torch.FloatTensor)))
                    ...
                    batch_len = Variable(torch.from_numpy(batch_len).type(torch.FloatTensor))
    ...
    def _test_time(self):
        ...
        for i in range(size):
            ...
            test_one_feed_data=Variable(torch.from_numpy(one_feed_data).type(torch.FloatTensor))
            test_position=Variable(torch.from_numpy(position).type(torch.FloatTensor))
            test_one_len=Variable(torch.from_numpy(np.array(one_len).reshape([1])).type(torch.FloatTensor))
    ...
    def _fit(self, train_iter, optimizer,train_idx):
        ...
        for i,x in enumerate(train_num):
            ...
            for j in range(random_len.shape[0]):
                ...
                batch_data.append(Variable(torch.from_numpy(one_feed_data).type(torch.FloatTensor)))
                batch_position.append(Variable(torch.from_numpy(one_position).type(torch.FloatTensor)))
        ...
        batch_rul_encoding = Variable(torch.from_numpy(batch_rul_encoding).type(torch.FloatTensor))
        batch_sequence_len = Variable(torch.from_numpy(batch_sequence_len).type(torch.FloatTensor))
        batch_rul = Variable(torch.from_numpy(batch_rul).type(torch.FloatTensor))
